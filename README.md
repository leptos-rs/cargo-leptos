...
# When using for example cross you need to be sure that the application import/passthough the environment variables that are generated by the build process.
# Read [during build](https://github.com/leptos-rs/cargo-leptos/?tab=readme-ov-file#during-build) about the variables that are used.
...
During the build several environment variables are "injected". With [cross](https://github.com/cross-rs/cross) on a [subset](https://github.com/cross-rs/cross/wiki/Configuration#environment-variable-passthrough) is passed into the container during the build.

For `cross` specific you need to add this section to the `Cross.toml` file:
...
Other tools need to accept the same environment variables. If this is not done correctly then the wasm files are not linked correctly. See [issue #574](https://github.com/leptos-rs/cargo-leptos/issues/574) for the details.
