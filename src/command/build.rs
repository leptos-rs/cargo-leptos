...
pub fn build_bin(cli: &Cli) -> Result<()> {
    ...
    let default_bin_cargo_args = vec![
        "build".to_string(),
        format!("--package={}", cli.bin_crate_name.clone().unwrap()),
        format!("--bin={}", cli.bin_crate_name.clone().unwrap()),
        "--no-default-features".to_string(),
        format!(
            "--target={}",
            cli.opts.bin_opts.bin_target_triple.clone().unwrap()
        ),
    ];
...
pub fn build_lib(cli: &Cli) -> Result<()> {
    ...
    let default_lib_cargo_args = vec![
        "build".to_string(),
        format!("--package={}", cli.lib_crate_name.clone().unwrap()),
        "--lib".to_string(),
        "--no-default-features".to_string(),
        format!("--target={}", cli.opts.lib_opts.lib_target_triple),
    ];
